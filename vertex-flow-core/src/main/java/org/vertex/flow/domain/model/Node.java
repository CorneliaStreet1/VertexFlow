...
public class Node {
     ...
     * 前置依赖节点的集合
     */
    private Set<Node> preDependNodes;
     ...
     * 需要与{@link Node#addPreNode}调用的顺序保持一致
     * 或者与{@link Node#addPreNodes}中可变参数的出现顺序保持一致
     */
    private List<Node> orderedOperatorInputNodes;
    ...
    public Node(@Nonnull IOperator operator, @Nonnull String nodeId) {
    ...
    private void initFields() {
        ...
        orderedOperatorInputNodes = Lists.newArrayList();
        init = false;
    }

    public Node addPreNode(Node node) {
        Objects.requireNonNull(node, "node cannot be null");
        Objects.requireNonNull(node.getNodeId(), "node id cannot be null");

        this.preDependNodes.add(node);
        this.orderedOperatorInputNodes.addLast(node);

        return this;
    }

    public Node addPreNodes(Node... nodes) {
        Objects.requireNonNull(nodes, "nodes cannot be null");
        for (Node node : nodes) {
            this.addPreNode(node);
        }
